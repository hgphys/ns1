"""
Facebookにおける友人関係ネットワークのスモールワールド性の検証
...
# 辺データの入力によるグラフの定義
G = nx.read_edgelist('data03/socfb-Haverford76.mtx', delimiter=' ', comments='%', create_using=nx.Graph)
...
plt.subplot(131)
nx.draw_networkx(G, pos, with_labels=False, node_color='blue', edge_color ='lightgrey',node_size=5)
plt.title('Data N={0}, L={1}'.format(N,L))

plt.subplot(132)
nx.draw_networkx(G_rand, pos, with_labels=False, node_color='blue', edge_color ='lightgrey', node_size=5)
plt.title('Degree Preserving Random N={0}, L={1}'.format(N_rand,L_rand))

plt.subplot(133)
nx.draw_networkx(G_comp, with_labels=False, node_color='blue', edge_color ='lightgrey', node_size=5)
