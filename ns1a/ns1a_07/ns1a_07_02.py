"""
ネットワーク科学研究者間の共著ネットワークのスモールワールド性の検証
...
# 辺データの入力によるグラフの定義
G = nx.read_edgelist('data02/ca-netscience.mtx', delimiter=' ', comments='%', create_using=nx.Graph)
...
plt.subplot(131)
nx.draw_networkx(G, pos, with_labels=False, node_color='lightblue', node_size=50)
plt.title('Data N={0}, L={1}'.format(N,L))

plt.subplot(132)
nx.draw_networkx(G_rand, pos, with_labels=False, node_color='lightblue', node_size=50)
plt.title('Degree Preserving Random N={0}, L={1}'.format(N_rand,L_rand))

plt.subplot(133)
nx.draw_networkx(G_comp, pos, with_labels=False, node_color='lightblue', node_size=50)
